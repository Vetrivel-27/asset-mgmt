...
function LogIn({ onLogin }) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const roleName = (data.user?.roleName || data.user?.role?.name || ROLES.EMPLOYEE).toLowerCase();

      // We maintain this fallback for specific legacy components that might still look for it directly
